...
class MultiMeter(object):
    ...
    def update(self, output, target, ):
        ...
        for i in range(5): # iterate over all classes
            pred = output.float() == i
            truth = target.float() == i
            self.tp[i] += pred.mul(truth).sum(2).sum(0).float()
            self.tn[i] += (1 - pred).mul(1 - truth).sum(2).sum(0).float()
            self.fp[i] += pred.mul(1 - truth).sum(2).sum(0).float()
            self.fn[i] += (1 - pred).mul(truth).sum(2).sum(0).float()
...
class BinaryMeter(object):
    ...
    def update(self, output, target):
        pred = output.float() >= 0.5

        truth = target.float() >= 0.5
        self.tp += pred.mul(truth).sum(2).sum(0).float()
        self.tn += (1 - pred).mul(1 - truth).sum(2).sum(0).float()
        self.fp += pred.mul(1 - truth).sum(2).sum(0).float()
        self.fn += (1 - pred).mul(truth).sum(2).sum(0).float()
